...
function draw() {
    ...
    if (bodies) {
        ...
        if (setting['house-system'] !== 'solar-sign' && setting['house-system'] !== 'solar') {
            ...
            bodies['DSC'] = { longitude: (bodies['ASC'].longitude + 180) % 360, longitude_speed: 0 };
            bodies['IC'] = { longitude: (bodies['MC'].longitude + 180) % 360, longitude_speed: 0 };
        } else {
            delete bodies['ASC'];
            delete bodies['MC'];
            delete bodies['DSC'];
...
function makeHouseList() {
    ...
    if (setting['house-system'] !== 'solar-sign' && setting['house-system'] !== 'solar') {
        ...
        $('<span class="return_date">').text(GetFullDateString(GetDateSunHasSpecificAngle(DSC, target_date))).appendTo(li_dsc);
        $('<span class="sabian">').text(SabianUtil.getSabianSymbol(DSC)).appendTo(li_dsc);
        ...
        $('<span class="return_date">').text(GetFullDateString(GetDateSunHasSpecificAngle(IC, target_date))).appendTo(li_ic);
        $('<span class="sabian">').text(SabianUtil.getSabianSymbol(IC)).appendTo(li_ic);
    }
...
function GetDateSunHasSpecificAngle(target_angle, date_from) {
    ...
    function getNext(date, diff_min) {
        if (diff_min !== 0 && Math.abs(diff_min * 60) < 1) diff_min = diff_min / Math.abs(diff_min) / 60;
        else if (diff_min !== 0 && Math.abs(diff_min) < 1) diff_min = diff_min / Math.abs(diff_min);
