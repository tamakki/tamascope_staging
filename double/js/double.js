...
$(function () {
    ...
    $('#btn_remove_setting').click(function() {
        if(confirm('設定を初期化します。よろしいですか？')){
            ...
            $('#body-table').empty();
            $('#aspect-table').empty();
            localStorage.removeItem('bodies_double');
            localStorage.removeItem('bodies2_double');
            localStorage.removeItem('casps_double');
            localStorage.removeItem('magnify_double');
            localStorage.removeItem('display-bodydata_double');
            localStorage.removeItem('display-aspect_double');
            localStorage.removeItem('display-opponent_double');
        }
    });
    $('#display-bodydata').change(function() {
        localStorage.setItem('display-bodydata_double', $('#display-bodydata').prop('checked'));
    });
    $('#display-aspect').change(function() {
        localStorage.setItem('display-aspect_double', $('#display-aspect').prop('checked'));
    });
    $('#display-opponent').change(function() {
        localStorage.setItem('display-opponent_double', $('#display-opponent').prop('checked'));
    });
...
function calc() {
    ...
    if(validate(setting)) {
        ...
        $.ajax({
            ...
            bodies = res;
            localStorage.setItem('bodies_double', JSON.stringify(bodies));
            $.ajax({
                url: 'https://acidic-chill-bat.glitch.me/api/horo',
                //url: 'http://localhost:3000/api/horo',
                type: 'post',
                ...
                bodies2 = res2;
                localStorage.setItem('bodies2_double', JSON.stringify(bodies2));
                try {
                    draw();
                } catch(ex) {
...
function getAspectTable (aspects){
    ...
    start_th.innerHTML = '<span>外<br>側<br>内側 ＼</span>';
    tr.append(start_th);
    for(key in bodies2) {
